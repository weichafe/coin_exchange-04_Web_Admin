        ...
        <router-view></router-view>
				...
				<div class="memberCheckMask" v-show="memberCheckMask">
					<div class="wrapper">
						<Steps :current="imgIndex">
								<Step title="" content="frente de la tarjeta de identificación"></Step>
								<Step title="" content="DNI en mano"></Step>
								<Step title="" content="parte posterior de la tarjeta de identificación"></Step>
						</Steps>
						<Row class="img">
						...
						<Row class="btns">
							<Row v-show="imgIndex!==2">
								<Col span="4" offset="10" >
									<Button type="primary" long @click="cancelMemberCheck">Cancelar</Button>
								</Col>
							</Row>

							<Row v-show="imgIndex===2">
								<Col span="4" offset="7" >
									<Button type="info" long @click="memberPass(true)">examen aprobado</Button>
								</Col>
								<Col span="4" offset="2" >
									<Button type="error" long @click="memberPass(false)">Auditoría no aprobada</Button>
								</Col>
							</Row>
							<Modal
								v-model="rejectModel"
								:width="450"
								title="Tenga en cuenta el motivo de la falla(opcional)"
								@on-ok="noPass"
			...
			<!-- </transition>	 -->
				<div class="businessCheckMask" v-show="businessCheckMask">
					<div class="wrapper">
						<Row class="title">
							<Col span="12" :class="{ active: !authImgIndex}">① Prueba de activos digitales personales</Col>
							<Col span="12" :class="{ active: !!authImgIndex}">② Prueba de transacción de activos digitales</Col>
						</Row>
						<Row class="businessInfo">
							<Col span="8">Número de teléfono：{{ businessCheckInfo.telno  }}</Col>
							<Col span="8">número de chat：{{ businessCheckInfo.wechat }}</Col>
							<Col span="8">número QQ：{{ businessCheckInfo.qq }}</Col>
							<Col span="8">Margen：{{ `${businessCheckInfo.amount}${businessCheckInfo.coinSymbol}` }}</Col>
						</Row>
						<Row class="img">
						...
						<Row class="btns">
							<Row v-show="authImgStep">
								<Col span="4" offset="10" >
									<Button type="primary" long @click="cancelBusinessCheck">Cancelar</Button>
								</Col>
							</Row>

							<Row v-show="!authImgStep">
								<Col span="4" offset="7" >
									<Button type="info" long @click="businessPass(true)">examen aprobado</Button>
								</Col>
								<Col span="4" offset="2" >
									<Button type="error" long @click="businessPass(false)">Auditoría no aprobada</Button>
								</Col>
							</Row>
							<Modal
