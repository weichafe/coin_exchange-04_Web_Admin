<template>
	<div class="home">
		<Row type="flex" justify="space-between" class="dashBoard">
			...
			<Col span="8">
			<Card>
				<span slot="title">comercio extrabursátil</span>
				<div class="cardBody">
					<p>Volumen total de comercio fiduciario</p>
					<p class="tradeTotalNum">{{ otcChart.borderInfo.amount }}</p>
					<p>volumen total de transacciones
						<span>{{ otcChart.borderInfo.money }}</span>
					</p>
					<p>tarifa total de manejo
				...
				<div class="cardFooterWrapper">
					<Row class="cardFooter">
						...
						<p>volumen de negociación de ayer
			...
			<Col span="8">
			<Card>
				<span slot="title">comercio de divisas</span>
				<div class="cardBody">
					<p>El volumen total de transacciones de transacciones de divisas.</p>
					<p class="tradeTotalNum">{{ exchangeChart.boardInfo.amount }}</p>
					<p style="line-height: 60px">
            tarifa total de manejo
		...
		<Row class="register mainChart">
			<Card>
				<h3>
          Cuadro comparativo de registro y certificación
					<span>(Fecha de consulta no más de 30 días）</span>
				</h3>
		...
		<Row class="otcVolume mainChart">
			<Card>
				<h3>Gráfico de tendencia del volumen de negociación de divisas fiduciarias
					<span>(Fecha de consulta no más de 30 días)</span>
				</h3>
				<Row class="chartFunction">
					<Col span="3" offset="15">
				...
				<Row v-show="!!otcChart.chartData.length">
					<v-chart :forceFit="true" :height="height" :data="otcChart.chartData">
						...
						<v-axis :dataKey="'tiempo'" :title="axisTitle"></v-axis>
						<v-axis :dataKey="'Volumen de comercio'" :title="axisTitle">
						</v-axis>
						<v-line position="时间*交易量" :color="'divisa'" />
						<v-point position="时间*交易量" shape="circle" :color="'divisa'" />
						<v-legend :color="'divisa'" />
					</v-chart>
				</Row>
		...
		<Row class="exchangeVolume mainChart">
			<Card>
				<h3>Gráfico de estadísticas de volumen de negociación de tokens</h3>
				<Row class="tradeArea">
					<p>
						<span class="title">Área de comercio</span>
		...
		<Row class="exchangeVolume mainChart">
			<Card>
				<h3>Gráfico estadístico del volumen de transacciones de divisas</h3>
				<Row class="tradeArea">
					<p>
						<span class="title">Área de comercio</span>
						<RadioGroup v-model="exchangeChart.tradeArea.active" @on-change="switchTradeCoin" size="large">
  ...
  data() {
    return {
      ...
      registerChart: {
        ...
        initTime: [],
        chartData: [],
        timeRange: { active: 0, time: ["tres días", "una semana", "enero"] },
        searchDate: {
        ...
        .then(res => {
          if (!res.code) {
            ...
            dv.transform({
              type: "rename",
              map: {
                registrationNum: "miembro registrado",
                applicationNum: "miembro de nombre real",
                bussinessNum: "Miembro Certificado",
                date: "tiempo"
              }
            });
            dv.transform({
              type: "fold",
              fields: ["miembro registrado", "miembro de nombre real", "miembro certificado"],
              key: "escribe",
