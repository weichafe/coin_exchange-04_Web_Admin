<template>
    ...
    <Card>
      ...
      <Row class="manageRow clearfix">
        <div class="manageWrapper">
          <Button type="error" @click="delAd">Eliminar</Button>
          <Button type="info" @click="addAd">añadir</Button>
        </div>
      </Row>
    ...
    <!-- 是否删除 -->
     <Modal
        ...
        @on-ok="confirmDel"
        @on-cancel="cancelDel">
        <p>si eliminar lo seleccionado{{ delArr.length }}项</p>
    </Modal>
		...
		<Modal
			...
			<p slot="header" style="color:#5cadff;text-align:center">
				...
				<span style="padding-left: 5px;" v-if="addAdStyle" >agregar anuncio</span>
				<span style="padding-left: 5px;" v-if="!addAdStyle" >ver anuncio</span>
			</p>
			<div class="addAdWrapper">
        <div v-if="!addAdStyle"><i>*</i> tiempo de creación：
          <Input v-model="adCreateTime" disabled/>
        </div>

        <p v-if="!addAdStyle"><i>*</i> número de anuncio：
          <Input v-model="adSerialNumber" disabled></Input>
        </p>

        <p><i>*</i> clasificación de anuncios：
          <InputNumber :max="100" :min="1" v-model="sort"></InputNumber>
        </p>

				<p><i>*</i> nombre del anuncio：
          ...
          <RadioGroup v-model="adStatus">
            ...
            <Radio label="1">
              <span>desconectado</span>
				...
				<div> <i>*</i> Colocación de anuncios：
          ...
          <RadioGroup v-model="adLang">
			...
			<Radio label="zh_HK">
			  <span>Chino tradicional</span>
				...
				<div><i>*</i> Tiempo de empezar：
					<DatePicker v-model="adStartTime" type="date" format="yyyy-MM-dd HH:mm:ss" placeholder="Por favor selecciona una hora" style="width: 200px"></DatePicker>
        </div>
				<div><i>*</i> Hora de finalización：
					<DatePicker v-model="adEndTime" type="date" format="yyyy-MM-dd HH:mm:ss" placeholder="Por favor selecciona una hora" style="width: 200px"></DatePicker>
					</div>

        <div><i>*</i> Imagen del anuncio：
        ...
        <p><i>*</i> Dirección de salto：
          ...
          <span v-if="listenBannerUrl">
            ...
            <Icon type="close-round" v-else></Icon>
				...
				<p><i>*</i> Notas publicitarias： <Input v-model="adRemark" type="textarea" :rows="4" placeholder="Agregar notas de anuncios..."></Input></p>
		  	</div>

        <div slot="footer" style="color:#5cadff;text-align:right">
          <div v-if="addAdStyle">
            <Button type="text" @click="cancelAddAd" >Cancelar</Button>
            <Button type="info" @click="confrimAddAd(true)" >confirmar</Button>
          </div>
          <Button type="info" @click="confrimAddAd(false)" v-if="!addAdStyle">renovar</Button>
        </div>
...
export default {
	data () {
      ...
      adLang:"zh_CN",
      adStatusArr: [
				{status: 0, statusName: 'en línea'},
				{status: 1, statusName: 'desconectado'}
			],
			adEndTime: '',
			addModal: false,
			adPosition: [
				{status: 0, posName: 'APP'},
				{status: 1, posName: 'PC'},
				{status: 2, posName: 'PC'}
      ],
			...
			columns_first: [
					...
					},
					{
						...
						render: (h, params) => {
							...
							if(sysAdLoc === 0) sysAdLocName = 'APP';
							else if(sysAdLoc === 1) sysAdLocName = 'pc ';
							else if(sysAdLoc === 2) sysAdLocName = 'pc ';
            ...
            render:(h, obj) => {
              ...
              if(link == "" || link ==undefined){
                ...
                h('p',{
					...
					{
						title: 'En línea/Fuera de línea',
						width: 90,
						render: (h, obj) => {
							let status=obj.row.status;
							let statuStr = String;

							if(status === 0) statuStr = 'en línea';
							else if(status === 1) statuStr = 'desconectado';
    ...
    refreshPageManual() {
      this.ifLoading = true;
      this.refreshTab({ pageNo: this.currentPageIdx, pageSize: 20 });
    },
    changePage(pageIndex){
      this.ifLoading = true;
      this.currentPageIdx = pageIndex;
      this.refreshTab({ pageNo: pageIndex, pageSize: 20 });
      // alert(pageIndex)
    },
    listenValName() {
