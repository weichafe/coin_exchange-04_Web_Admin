<template>
  <div>
    <Card>
      ...
      <Row class="functionWrapper" style='padding:0px 8px 8px 8px'>
        <Col span="4">
          <div style="float: left" class="clearfix">
            <div class="poptip">Seleccione un par comercial de control：</div>
        ...
        <Col span="8">
          <div class="poptip">Estrategia de panel：</div>
            <RadioGroup v-model="strategy" type="button" size="small">
                  <Radio label="1">sigue la tendencia</Radio>
                  <Radio label="2">tendencia personalizada</Radio>
            </RadioGroup>
        </Col>

        <Col span="12" v-show="strategy == '2'">
          <div style="float: right" class="clearfix">
            <div class="poptip">granularidad del control：</div>
            <RadioGroup v-model="period" type="button" size="small">
                  <Radio label="1">15 m</Radio>
                  <Radio label="2">30 m</Radio>
                  <Radio label="3">1 hora</Radio>
                  <Radio label="4">2 horas</Radio>
                  <Radio label="5">4 horas</Radio>
            </RadioGroup>
          </div>
          <div style="float: right;margin-right: 20px;" class="clearfix">
            <div class="poptip">Precio máximo del día：</div>
            <Input v-model="maxPrice" style="width: 150px" size="small">
                <Button slot="append" type="primary" @click="setMaxPrice()">确定</Button>
            </Input>
          </div>
          <div style="float: right;margin-right: 20px;" class="clearfix">
            <div class="poptip">seleccione fecha：</div>
      ...
      <div v-show="strategy == '1'" style="width: 100%;background: #F5F5F5;">
        <Row style='padding:20px 8px 10px 8px'>
          <Col span="24">
            <p style="font-size: 12px;color: red;padding-left: 30px;margin-bottom: 20px;">Si elige [Siguiendo tendencia], confirme si se ha creado un robot para el par comercial del panel de control. Se recomienda crear primero un robot de panel de control y configurarlo para que se detenga, y luego iniciar el robot después de configurarlo para seguir la tendencia.
            </p>
            <Form :label-width="130">
              <FormItem label="seguir el par comercial：">
                ...
                </Select><br>Después de la selección, el mercado seguirá la tendencia del mercado del par comercial seleccionado. Actualmente solo admite BTC/USDT, ETH/USDT, LTC/USDT, si necesita más pares comerciales, se requiere un desarrollo personalizado.
              </FormItem>

              <FormItem label="seguir la relación de precios：">
                  <Input v-model="flowPercent" style="width: 150px;">
                      <span slot="append">%</span>
                  </Input>Calcule el último precio del mercado actual de acuerdo con la transacción seleccionada. Por ejemplo: si el valor se establece en 1, la moneda se anclará al 1% del precio de mercado BTC/USDT y la tendencia será consistente con BTC. /USDT.
              </FormItem>
      ...
      <div  v-show="strategy == '2'">
        ...
        <p style="padding: 10px 0 10px 0;"><Button size="small" type="primary" @click="generateKLine()">Dibujar gráfico de velas de vista previa</Button><span style="float:right;font-size:12px;">La línea K simulada generada a continuación es solo para referencia. El resultado real de la ejecución es similar a la siguiente figura, pero no exactamente igual.</span></p>
        ...
        <Row class="functionWrapper" style='padding:0px 8px 0px 8px;margin-top: 20px;'>
          ...
          <Col span="18">
            <div style="float: left;" class="clearfix">
              <Form :label-width="130">
                <FormItem label="Relación de fluctuación de precio permitida：">
          ...
          <Col span="6">
            <div style="float: right" class="clearfix">
              <Button type="success" @click="commitRobot()">Guardar datos de tendencia de robot</Button>
            </div>
          </Col>
        </Row>
      </div>
    ...
    <Card style="margin-top: 10px;"  v-show="strategy == '2'">
      <p slot="title">
        BTC/USDT Datos de tendencias de robots de pares comerciales
      </p>
      <div>
        <Row>
          <Table
   ...
   methods: {
        ...
        getRobotKdataList(obj).then( res => {
          if(!res.code) {
            if(res.data.length > 0) {
              ...
              this.$Notice.error({
                    title: "No hay datos de tendencias de velas japonesas actuales y futuras para este par comercial",
                    desc: res.message,
                    duration: 10
                });
            }
     ...
     getCoinPairList(){
      ...
      getCustomCoinPairList(obj).then( res => {
          if(!res.code) {
            if(res.data.length  == 0){
              this.$Notice.error({
                    title: "No se puede obtener el par comercial de control",
                    desc: res.message,
                    duration: 10
                });
            }else{
     ...
     getRobotParams(symbol){
        ...
        getRobotConfig(obj).then( res => {
              if (!res.code) {
                  ...
                  this.$Notice.info({
                        title: "robot",
                        desc: "No hay ningún robot para este par comercial, cree un nuevo robot en [Administración de monedas]",
                        duration: 10
                    });
              }
            });
     ...
     generateKLine(){
        ...
        for(var i = 0; i <= 96; i++) {
          ...
          var priceRandClose = Math.ceil(Math.random()*this.pricePencent);
          var priceRandHight = Math.ceil(Math.random()*this.pricePencent);
          var priceRandLow = Math.ceil(Math.random()*this.pricePencent);
          this.klineMockYData[i][0] = i > 0 ? this.klineMockYData[i-1][1] : this.storeData[i][1];
          ...
          if(this.klineMockYData[i][0] > this.klineMockYData[i][1]){
