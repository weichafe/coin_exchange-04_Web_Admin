   ...
   <div>
      <div class="examin_mian">
        <Row type="flex" justify="center" class="code-row-bg e_mianinput">
          <Col span="2"> Nombre del cliente : </Col>
          <Col span="8"> 
            <span>{{ memberName }}</span>
          </Col>
        </Row>
        <Row type="flex" justify="center" class="code-row-bg e_mianinput">
          <Col span="2"> Número de identificación : </Col>
          <Col span="8">
            <span>{{ IDCard }}</span>
          </Col>
        </Row>
        ...
        <Row v-if="!status">
          <Row type="flex" justify="center" class="code-row-bg" style='margin-top:20px;text-aligin: center'>
            <Col span="6" >
              <RadioGroup v-model="ifPass" @on-change="tabSwitch = !tabSwitch">
                <Radio label="examen aprobado"></Radio>
                <Radio label="Reseña rechazada" style='margin-left:35px'>
                </Radio>
              </RadioGroup>
            </Col>
          </Row>
          <Row type="flex" justify="center" class="code-row-bg" style='margin-top:20px'>
            ...
            <div style="text-align: center">
              <Row  type="flex" justify="center" class="code-row-bg" v-show="!tabSwitch">
                <Col span="4" style='font-size:16px'>Motivo del rechazo :</Col>
                <Col span="16"> <Input v-model="rejectReason" type="textarea" :rows="4" ></Input></Col>
              </Row>
              <Row  type="flex" justify="center" class="code-row-bg" v-show="tabSwitch">
                <Col span="8" style='font-size:16px;color:#2287fe'>examen aprobado</Col>
              </Row>
            </div>
            </Col>
          </Row>
          <Row type="flex" justify="center" class="code-row-bg" style='margin-top:20px'>
            <Col span="6">
              <Button @click="pass" type="success" long size="large">confirmar</Button>
            </Col>
          </Row>
        </Row>
        <Row v-if="!!status" class="passInfo">
          <p v-if="status === 2">Examen aprobado</p>
          <p v-if="status === 1" style="color: #ed3f14">Revisión fallida</p>
        </Row>
...
export default {
  ...
  return {
    rejectModel: false,
      imgAltArr: ['frente de la tarjeta de identificación','documento de mano', 'parte posterior de la tarjeta de identificación'],
      clickIndex: null,
      ifEnlarge: false,
      memberName: null,
      IDCard: null,
